export const setupConvertRegionsFeature = (deps = {}) => {
  ...
  bindClick(el.btnConvertRegionsToScreens, () => {
    const r = cur();
    if (!r || isRectLocked(r) || isNoteRect(r)) return;
    ...
    if (regs.length <= 1) { showMessageModal("Недостаточно регионов для преобразования"); return; }
    ...
    for (const rg of regs) {
      ...
      const localCols = Math.max(1, c1 - c0), localRows = Math.max(1, r1 - r0), localHidden = [];
    ...
    setSelection(created.map(it => it.id), created[created.length - 1].id);

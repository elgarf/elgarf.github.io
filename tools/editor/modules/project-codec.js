...
export const createProjectCodec = (deps) => {
  ...
  const PROJECT_QUERY_KEY_MAP = Object.freeze({
    ...
    installLayers: "il", contours: "cn", text: "tx", flow: "fw", devices: "dv",
    ...
    rig: "rg", locked: "lk", kind: "kd", noteText: "nt", noteIncludeInArtRender: "nar", shapeOpacity: "so", shapePoints: "shp", type: "tp", inX: "inx", inY: "iny", outX: "oux", outY: "ouy", projectCache: "pc",
  ...
  const normalizeProjectSchema = (raw) => {
    ...
    out.settings.installLayers = {
      ...
      devices: out.settings.installLayers.devices !== false,
  ...
  const compactProjectForQuery = data => {
    ...
    if (isPlainObject(out.settings)) {
      ...
      if (isPlainObject(out.settings.installLayers)) {
        ...
        if (il.devices === true) delete il.devices;
